...
## 看板置かれた
	execute if entity @s[nbt=!{data:{HdP:{ASign:{Put:1b}}}}] run data modify entity @s data.HdP.ASign.Put set value 1b
## 看板更新
	execute if entity @s[nbt={data:{HdP:{ASign:{Enable:1b}}}}] run function hd.prkr:object/attractive.sign/change
	execute if entity @s[nbt={data:{HdP:{ASign:{Enable:0b}}}}] run data modify block ~ ~ ~ front_text merge value {has_glowing_text:0b,color:"black"}
	execute if entity @s[nbt={data:{HdP:{ASign:{Enable:0b}}}}] run data modify block ~ ~ ~ back_text merge value {has_glowing_text:0b,color:"black"}
## 看板にコマンドを代入
