...
## 一番近くの物を消す
	execute if data storage hd.p: OffHandCopy.tag.HdP.Laser store success score $HdP.Success HdP.Temp run kill @e[type=marker,tag=HdP.Laser,distance=..4,sort=nearest,limit=1]
	...
	execute if data storage hd.p: OffHandCopy.tag.HdP.ASign store success score $HdP.Success HdP.Temp run kill @e[type=marker,tag=HdP.ASign,distance=..4,sort=nearest,limit=1]
