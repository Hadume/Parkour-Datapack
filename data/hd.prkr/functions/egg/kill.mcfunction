...
## 一番近くの物を消す
	...
	execute if data storage hd.p: OffHandCopy.tag.HdP.CGArea store success score $HdP.Success HdP.Temp run kill @e[type=minecraft:marker,tag=HdP.CGArea,distance=..4,sort=nearest,limit=1]
	execute if data storage hd.p: OffHandCopy.tag.HdP.ASign store success score $HdP.Success HdP.Temp run kill @e[type=minecraft:marker,tag=HdP.ASign,distance=..4,sort=nearest,limit=1]
