...
## 近くのオブジェクトにTagを付ける
	execute if data storage hd.p: SelectedItemCopy.tag.HdP.Laser as @e[type=minecraft:marker,tag=HdP.Laser,distance=..2,sort=nearest,limit=1] run tag @s add HdP.This
	execute if data storage hd.p: SelectedItemCopy.tag.HdP.Start as @e[type=minecraft:marker,tag=HdP.Start,distance=..2,sort=nearest,limit=1] run tag @s add HdP.This
## プレイヤーの向きに合わせる
